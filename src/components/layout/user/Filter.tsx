...
import {
  CategoryTabs,
  CurrentBetCategoryTabsContainer,
} from "../../currentBets/styledComponents";
import { colorHex } from "../../../utils/constants";
...
interface Props {
  totalBet: number;
  totalStake: number;
  searchFilters: FilterCurrentBets;
  setSearchFilters: Dispatch<SetStateAction<FilterCurrentBets>>;
}
const Filter = ({
  searchFilters,
  setSearchFilters,
    ...
    <Box py={{ xs: 1, md: 2, lg: 1 }}>
      ...
      >
        <Grid container rowGap={0.5} display="flex" alignItems="center">
          ...
          <Grid item textAlign={"left"} xs={12} md={6} fontSize={"0.8rem"}>
            Total Bets:
